...
function validateForm() {
  ...
  if (usernameInput.value.trim() == "") {
    ...
    usernameInput.value.trim().length > 25
  ) {
    setError(
      usernameInput,
      "Le nom doit contenir au moins 3 et au maximum 25 caractères"
    );
  } else {
  ...
  if (passwordInput.value.trim() == "") {
    ...
    setError(
      passwordInput,
      "Le mot de passe doit comporter au moins 8 caractères, dont au moins 1 caractère minuscule, 1 caractère majuscule, 1 chiffre et 1 caractère spécial dans (!@#$%^&*)"
    );
  }
